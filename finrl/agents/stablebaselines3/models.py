...
class DRLEnsembleAgent:
    ...
    def run_ensemble_strategy(
        self, A2C_model_kwargs, PPO_model_kwargs, DDPG_model_kwargs, timesteps_dict, turbulence_threshold=None
    ):
        ...
        for i in range(
            ...
            print(f"======A2C Training===={self.train_period[0]} to {self.unique_trade_date[i - self.rebalance_window - self.validation_window]}====")
            self.model_a2c = self.get_model(
                "a2c", self.train_env, policy="MlpPolicy", seed=self.seed, model_kwargs=A2C_model_kwargs
            )
            self.model_a2c = self.train_model(
                self.model_a2c,
                "a2c",
            ...
            print(f"======PPO Training===={self.train_period[0]} to {self.unique_trade_date[i - self.rebalance_window - self.validation_window]}====")
            self.model_ppo = self.get_model(
                "ppo", self.train_env, policy="MlpPolicy", seed=self.seed, model_kwargs=PPO_model_kwargs
            )
            self.model_ppo = self.train_model(
                self.model_ppo,
                "ppo",
            ...
            print(f"======DDPG Training===={self.train_period[0]} to {self.unique_trade_date[i - self.rebalance_window - self.validation_window]}====")
            self.model_ddpg = self.get_model(
                "ddpg",
                self.train_env,
                policy="MlpPolicy",
                seed=self.seed,
            ...
            self.model_ddpg = self.train_model(
                self.model_ddpg,
                "ddpg",
            ...
            if (sharpe_ppo >= sharpe_a2c) & (sharpe_ppo >= sharpe_ddpg):
                ...
                model_ensemble = self.model_ddpg

            # Model selection using average action ensembling
            model_averageaction = AverageAction([self.model_ppo, self.model_a2c, self.model_ddpg])
            # TODO: Need to create a model_use
            ...
            # Trading starts
            # We will trade with both models
            print(
                "======Trading from: ",
                self.unique_trade_date[i - self.rebalance_window],
                "to ",
            ...
            last_state_averageaction = self.DRL_prediction(
                model=model_averageaction,
                name="averageaction",
                last_state=last_state_averageaction,
                iter_num=i,
                turbulence_threshold=turbulence_threshold,
                initial=initial,
            )
...
class AverageAction:
    ...
    def predict(self, env):
        ...
        for model in self.models:
            a_, _ = model.predict(env)
            actions.append(a_)

        # Find majority vote if any
        actions_array = np.array(actions)
        # print(actions_array)
        # print(actions_array.shape)

        average_action = np.mean(actions_array, axis=0)
        # Because we are taking the average, the average of 3 normalized
        # actions will always end up with a normalized value that's
        # within [-1, 1].

        # Check dimensions to ensure that we always return 1 more extra
        # dimension
        if len(average_action.shape) == 1:
            average_action = np.expand_dims(average_action, axis=0)
        elif len(average_action.shape) == 2:
            pass
        else:
            raise Exception("Invalid action shape detected!")

        # print(average_action.shape)
        # print(actions[0].shape)
        assert average_action.shape == actions[0].shape, "Incorrect output shape detected!"

        return average_action, None
